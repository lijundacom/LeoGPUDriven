...
public class BuildMinMaxHeightMapHex : EditorWindow
{
    ...
    [MenuItem("GPUDriven/BuildMinMaxHeightMapHex")]
    public static void CreateEditorWindow()
    {
        EditorWindow.GetWindow(typeof(BuildMinMaxHeightMapHex));
    }
    ...
    private void BuildMinMaxMap()
    {
        ...
        RenderTextureDescriptor outputRTDesc = new RenderTextureDescriptor(mOutputTextureSize.x, mOutputTextureSize.y, RenderTextureFormat.R16, 0, 1);
        ...
        for (int i = 1; i < mOutputMipCount; i++)
        {
            ...
            RenderTextureDescriptor inputRTDesc = new RenderTextureDescriptor(destTexSize.x * 2, destTexSize.y * 2, RenderTextureFormat.R16, 0, 1);
        ...
        Texture2D texture2D = new Texture2D(outputRT.width, outputRT.height, TextureFormat.R16, mOutputMipCount, false);
    ...
    private void ReadRenderTexture(RenderTexture renderTexture, Texture2D tex2D, int mip, int mipcount, List<int> readResult, Action callback)
    {
        ...
        {
            tex2D.SetPixelData(req.GetData<float>(), mip);
            readResult.Add(mip);
    ...
    private string BuildMinMaxHeightMapCS = "Assets/ComputeShader/BuildMinMaxHeightMapHex.compute";
